...
def _machine_response(
    *,
    ...
    return {
        ...
        "machine": {
            "app": validation.get("fly_app_name") or settings.fly_app_name,
            "region": validation.get("fly_region") or region,
            "fallback_regions": validation.get("fly_fallback_regions") or fallbacks,
            "lease_key": validation.get("lease_key") or lease_key,
            "ttl_expires_at": validation.get("ttl_expires_at") or ttl_expires_at,
            "labels": validation.get("labels") or labels,
        },
    }
...
def evaluate_scorecard(
    ...
    if metrics.run_days < VALIDATION_DAYS_REQUIRED:
        reasons.append(f"run lasted {metrics.run_days} days; required {VALIDATION_DAYS_REQUIRED}")
...
async def provision_validation(
    ...
    if existing_rows:
        return _machine_response(
            validation=existing_rows[0],
            settings=settings,
            region=region,
            fallbacks=fallbacks,
            lease_key=lease_key,
            ttl_expires_at=existing_rows[0].get("ttl_expires_at"),
            labels=labels,
        )
    ...
    already_spent = sum(int(row.get("estimated_cost_cents") or 0) for row in _rows(spend))
    ...
    expires_at = now + dt.timedelta(hours=settings.validation_machine_ttl_hours)
    row = {
        "user_id": user_id,
        "strategy_id": body.strategy_id,
    ...
    inserted = sb.table("validation_runs").insert(row).execute()
    validation = (_rows(inserted) or [row])[0]
    return _machine_response(
